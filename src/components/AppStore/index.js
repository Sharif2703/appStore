...
class AppStore extends Component {
  state = {tabCategory: 'SOCIAL', inputTextField: ''}

  categoryCriteria = () => {
    const {tabCategory} = this.state
    const filterResults = appsList.filter(each => each.category === tabCategory)
    return filterResults
  }

  collectInputText = event => {
    this.setState({inputTextField: event.target.value})
  }

  changeCategory = id => {
    this.setState({tabCategory: id})
  }

  render() {
    const {inputTextField} = this.state
    const categoryCriteria = this.categoryCriteria()

    const filterSearch = categoryCriteria.filter(every =>
      every.appName.toLowerCase().includes(inputTextField.toLowerCase()),
    )
    ...
    return (
      <div className="app-store-bg">
        <div>
          ...
          <div className="search-box">
            <input
              ...
              onChange={this.collectInputText}
            />
            <img
        ...
        <ul className="unordered-list-con">
        ...
        <ul className="unordered-list-app-con">
          {filterSearch.map(eachAppItem => (
            <AppItem key={eachAppItem.appId} appItem={eachAppItem} />
          ))}
        </ul>
      </div>
    )
